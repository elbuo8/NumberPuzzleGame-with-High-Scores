...
public class Controls extends JPanel implements KeyListener, ActionListener{
	 ...
	 */
	public Controls(int size, String name) 
	{
		this.size = size;
	 ...
	 */
	public void draw(Graphics2D g2) throws UnknownHostException, MongoException {
		setFontSize();
	 ...
	 * @throws UnknownHostException 
	 */
	public void verify() throws UnknownHostException, MongoException{
		boolean sorted = true;
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
	...
	private void addHS() throws UnknownHostException, MongoException {
		Mongo mongo = new Mongo("localhost", 27017);
		DB db = mongo.getDB("npg");
		DBCollection collection = db.getCollection("hs");
		BasicDBObject player = new BasicDBObject();
		player.put("name", name);
		player.put("moves", moves);
		collection.insert(player);

		DBCursor cursor = collection.find().sort(new BasicDBObject("moves", 1)).limit(5);
		while (cursor.hasNext()) {
			DBObject top = cursor.next();
